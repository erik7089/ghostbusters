...
class ChatClient(Frame):
  ...
  # Initialize UI - Configure frame parameters(size, position, inner objects)
  def initUI(self):
    windowColor = "#99FF33"
    s = Style()
    s.configure('Custom.TFrame', background=windowColor)
    ...
    self.root.configure(background=windowColor)
    
    padX = 10
    padY = 10
    parentFrame = Frame(self.root, style="Custom.TFrame")
    parentFrame.grid(padx=padX, pady=padY, stick=E+W+N+S)
    
    ipGroup = Frame(parentFrame, style="Custom.TFrame")
    ...
    clientRemoveButton = Button(ipGroup, text="Remove", width=10, command=self.handleRemoveClient)
    ...
    clientRemoveButton.grid(row=0, column=9, padx=5)
    
    readChatGroup = Frame(parentFrame, style="Custom.TFrame")
    ...
    writeChatGroup = Frame(parentFrame, style="Custom.TFrame")
    ...
    self.statusLabel = Label(parentFrame, style="Custom.TFrame")

    bottomLabel = Label(parentFrame, text="", style="Custom.TFrame")
  ...
  # Method in a separate thead to check for new clients
  ...
  def handleRemoveClient(self):
    if (self.friends.curselection()):
      addr = self.friends.get(self.friends.curselection()[0])
      clientsoc = self.clientMap[addr]
      del self.clientMap[addr]
      self.friends.delete(self.friends.curselection()[0])
      del self.allClients[clientsoc]
      clientsoc.close()

  # Method in a separate thead to check for new messages from particular user(clientsoc, clientaddr)	
  ...
  # Appends sent or received message to listbox and saves to log file
  ...
  def addClient(self, clientsoc, clientaddr):
    print(clientsoc)
    self.clientMap["%s:%s" % clientaddr] = clientsoc
  ...
  def removeClient(self, clientsoc, clientaddr):
      # python 3: for key, value in self.clientMap.iteritems():
      for key, value in self.clientMap.items():
        if (value == clientsoc):
          del self.clientMap[key]
...
# Populates chat listbox with log data
def initChat(app):
  if (os.path.isfile("chat.log")):
    fh = open("chat.log", "r") 
    chatList = fh.readlines() 
    app.receivedChats.config(state=NORMAL)
    for item in chatList:
      app.receivedChats.insert("end", item)
    app.receivedChats.config(state=DISABLED)
